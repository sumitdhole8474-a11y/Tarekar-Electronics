...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!blog) {
    return {
      title: "Blog Not Found | Tarekar Electronics",
    };
  }

  const blogTitle = blog.title || "Tarekar Electronics";

  const title = `Know more about ${blogTitle} | Tarekar Electronics`;

  const description =
    blog.description ||
    `Read more about ${blogTitle} from Tarekar Electronics and stay updated with our latest insights and services.`;
  ...
  return {
    ...
    keywords: [
      blogTitle,
      "Tarekar Electronics Blog",
      "Amaravati services updates",
      "Latest updates Tarekar Electronics",
    ],
  };
}
type Props = {
